    ...
    : m_seed(0)
{
    ...
    if (configHelper.ShouldRandomize())
    {
        ...
        randomizer = std::make_shared<BlockRandomizer>(0, 1, deserializer, ioPrefetch, multithreadedGetNextSequences, useLocalTimeline);
    }
    else
    {
        randomizer = std::make_shared<NoRandomizer>(deserializer, multithreadedGetNextSequences, useLocalTimeline);
    }
